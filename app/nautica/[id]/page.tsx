...
import { formatPrice, getSimilarVessels } from "@/lib/vessels";
...
export async function generateMetadata({
  ...
  if (!vessel) return { title: "Embarcación no encontrada" };
  return {
    title: vessel.title,
    description: `${vessel.location} · ${formatPrice(vessel.price, vessel.operation)} · ${vessel.lengthMeters} m`,
  };
}
          ...
          <div className="mt-16 border-t border-accent/15 pt-12 sm:mt-20 sm:pt-16">
            <RevealOnScroll>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <SectionHeading>Embarcaciones similares</SectionHeading>
                <p className="text-sm text-white/45 sm:pb-1">
                  Selección en la misma zona u operación
                </p>
              </div>
            </RevealOnScroll>
