...
export function ServiceLanding({ page }: ServiceLandingProps) {
  ...
  const highlight = page.highlight
    ? tPages(`${slug}.highlight` as never)
    : null;
  const extraLinkLabel = page.extraLink
    ? tPages(`${slug}.extraLink` as never)
    : null;
  const privacyNote = page.privacyNote
    ? tPages(`${slug}.privacyNote` as never)
    : null;
